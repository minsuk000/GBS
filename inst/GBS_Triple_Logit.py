...
def D(y, X, Theta):
  ...
  out =  -1.0*(1-y)*c - torch.log(1.0+torch.exp(-c))
  return out
...
optimizer = torch.optim.RMSprop(G.parameters(), lr= 0.001, alpha=0.99, eps=1e-08)
...
U1 = int(K0*0.6)
U2 = int(K0*0.2)
a0_sample = torch.distributions.exponential.Exponential(torch.ones(K0-U, n_a))
a1_sample = torch.distributions.exponential.Exponential(torch.ones(U1, n_a))
a2_sample = torch.distributions.exponential.Exponential(torch.ones(U2, n_a))

while J == 1:
    for param_group in optimizer.param_groups:
        param_group["lr"] = param_group["lr"]/((it0+1.0)**lr_power)
    weight1 = a0_sample.sample().to(device)
    m = torch.mean(weight1,1).to(device)
    m = m.reshape(K0-U,1)
    weight1 = weight1/m
    weight2 = torch.distributions.gamma.Gamma(weight1, torch.ones(K0-U, n_a).to(device)).sample()
    m = torch.mean(weight2,1).to(device)
    m = m.reshape(K0-U,1)
    weight2 = weight2/m
    weight3= torch.distributions.gamma.Gamma(weight2, torch.ones(K0-U, n_a).to(device)).sample()
    m = torch.mean(weight3,1).to(device)
    m = m.reshape(K0-U,1)
    weight3 = 1.1*weight3
    
    alpha[range(U1+U2,n_a),:] = weight3 
    
    weight1 = a1_sample.sample().to(device)
    #m = torch.mean(weight1,1).to(device)
    #m = m.reshape(U1,1)
    weight1 = 1.1*weight1
    alpha[range(0,U1),:] = weight1
    weight1 = a2_sample.sample().to(device)
    m = torch.mean(weight1,1).to(device)
    m = m.reshape(U2,1)
    weight1 = weight1/m
    weight2 = torch.distributions.gamma.Gamma(weight1, torch.ones(U2,n_a).to(device)).sample()
    #m = torch.mean(weight2,1).to(device)
    #m = m.reshape(U2,1)
    weight2 = 1.1*weight2
    #k = sample(range(10,15),1)
    alpha[range(U1,U1+U2),:] = weight2

    w1 = torch.matmul(alpha, A).t()
    Theta = G(alpha)
    Theta[Theta != Theta] = 0.0
    loss1 = D(y, X, Theta).to(device)
    loss_log = loss1*w1
